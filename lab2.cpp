   	...
   	solution.push_back(tocka_kraj);
		while(true) {
			if(isSafe(tocka_kraj.first,tocka_kraj.second-1,N,M) && isSafe(tocka_kraj.first-1,tocka_kraj.second,N,M)) {
				if(acc_cost[tocka_kraj.first-1][tocka_kraj.second] >= acc_cost[tocka_kraj.first][tocka_kraj.second-1]) {
...
int main() { 
    ...
    uint16_t polje[] = {100, 200, 1000, 0,400,200,
                        200, 100,  600, 0,800,100,
                        300, 1600, 100, 0,300,900,
						300, 1600, 1000, 4000,3000,900,
						300, 400, 100, 400,300,900,
						1600, 100, 100, 400,300,900
						};
    uint16_t *ptr;
    ptr = polje;
    std::vector<std::pair<unsigned int, unsigned int> > solution;

    uint16_t brCvijeca = solve(6, 6, ptr, solution);
